...

{
    ...
    //in vec4 vBoneWeights;				// For bump mapping X,Y,Z (W ignored)
    ...
    //in vec4 vBoneIDs;				// For bump mapping X,Y,Z (W ignored)
...
bool LoadPLYModelFromFile(std::string fileName, sModelDrawInfo& drawInfo)
{
    ...
    //TODO: Replace with actual bone loading when I switch to assimp. For now, fake it.
    if (fileName == "assets/models/tentacle.ply") {
        for (unsigned int index = 0; index < vecVertexArray.size(); index++) {
            float tmpBoneId = 0;
            if (vecVertexArray[index].y < 1.f)
                tmpBoneId = 0;
            else if (vecVertexArray[index].y < 5)
                tmpBoneId = 1;
            else if (vecVertexArray[index].y < 9)
                tmpBoneId = 2;
            else
               tmpBoneId = 3;
            vecVertexArray[index].boneId = tmpBoneId;
            vecVertexArray[index].boneWeight = 1.f;
        }
    }
