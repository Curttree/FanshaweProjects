...
void cLightManager::DayTimeStep(float deltaTime) {
	...
	if (isDayTime && elapsedDayTime >= sunset + timeToAdjust) {
		...
		float howFar = (deltaTime) / (sunset + timeToAdjust);
		theLights[0].diffuse.a -= (1.75f * howFar);
		theLights[6].diffuse.a -= (1.75f * howFar);
		theLights[7].diffuse.a -= (1.75f * howFar);
		theLights[8].diffuse.a -= (1.75f * howFar);
	}
	if (isDayTime) {

		theLights[6].position += glm::vec4(3.f, 0.f, 2.f, 0.f);
		theLights[7].position += glm::vec4(3.f, 0.f, 2.f, 0.f);
		theLights[8].position += glm::vec4(3.f, 0.f, 2.f, 0.f);
	}
...
void cLightManager::FlickerCandles() {
	for (unsigned int x = 9; x < NUMBER_OF_LIGHTS; x++) {
		theLights[x].atten = (float)(0.75 + static_cast<float>(rand()) / (static_cast<float>(RAND_MAX) / (1.25 - 0.75))) * glm::vec4(0.003f, 0.001f, 0.0027f, 400.f);
	}
}
