...
cAITank::cAITank(cMesh* _model) {
	...
	laserBubble = new cMesh();
	laserBubble->meshName = "Sphere_xyz_n_rgba.ply";
	laserBubble->positionXYZ = model->positionXYZ;
	laserBubble->scale = 0.f;
	laserBubble->bUseWholeObjectDiffuseColour = true;
	laserBubble->wholeObjectDiffuseRGBA = startingColour;
	::g_vec_pMeshes.push_back(laserBubble);
}

void cAITank::TimeStep(float deltaTime) {
	if (state == TankState::DEAD || !::g_gameStarted) {
		return;
	}
	if (laserBubble->positionXYZ != model->positionXYZ) {
		laserBubble->positionXYZ = model->positionXYZ;
	}
	if (activeBullet) {
	...
	if (state == TankState::CHARGING) {
		...
		laserBubble->scale = currentLaserChargeTime / maxLaserChargeTime;
	...
	if (theMessage.command == "DESTROY BULLET") {
		...
		if (state != TankState::DEAD) {
			...
			if (health <= 0.f) {
				...
				sMessage promoteMessage;
				promoteMessage.command = "PROMOTE TANK";
				promoteMessage.vec_iData.push_back(theMessage.vec_iData[0]); 
				p_Mediator->RecieveMessage(promoteMessage);
			}
		}
	}
	...
	if (responseMessage.command == "FIRE AWAY") {
		...
		if (heading.x > 0.f) {
			...
			for (int x = 1; x <= dist; x++) {
				...
				cLaser* newLaser = new cLaser(GetId(), laserPos, startingColour, damageModifier);
