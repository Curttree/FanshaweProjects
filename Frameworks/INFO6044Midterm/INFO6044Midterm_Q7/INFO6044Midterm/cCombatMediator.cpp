...
bool cCombatMediator::RecieveMessage(sMessage theMessage) {
	if (theMessage.command == "INITIALIZE GAME") {
		...
		ValidateBulletPosition(theMessage.vec_iData[0], theMessage.vec_v4Data[0], theMessage.vec_sData[0]);
	}
	...
	else if (theMessage.command == "PROMOTE TANK") {
		for (int x = 1; x < tanks.size(); x++) {
			if (tanks[x]->GetId() == theMessage.vec_iData[0]) {
				PromoteTank(x);
			}
		}
	}
		...
		for (iTank* tank : tanks) {
			if (tank->GetId() == theMessage.vec_iData[0]) {
				...
				if (theResponse.command == "CRASH" && theResponse.vec_iData.size() > 0) {
					...
					if (theMessage.vec_sData[0] == "BULLET") {
						CleanupBullet(theMessage.vec_iData[0]);
					}
					else if (theMessage.vec_sData[0] == "LASER") {
						CleanupLaser(theMessage.vec_iData[1], theMessage.vec_iData[0]);
					}
					else if (theMessage.vec_sData[0] == "SUPERBULLET") {
						CleanupSuperBullet(theMessage.vec_iData[0]);
					}
				}
				break;
			}
...
void cCombatMediator::ValidateBulletPosition(int ownerID, glm::vec3 position, std::string type) {
	if (position.x > 60 || position.x < 0 || position.y > 100 || position.y < 0) {
		...
		else if (type == "SUPERBULLET2") {
			CleanupSuperBullet(ownerID);
		}
		else if (type == "SUPERBULLET1") {
			for (iTank* tank : tanks) {
				if (tank->GetId() == ownerID) {
					sMessage bulletMessage;
					bulletMessage.command = "CURVE SUPERBULLET";
...
void cCombatMediator::CleanupSuperBullet(int ownerID) {
	// Know there is a more scalable way to do this, but I am pressed for time and number of tanks is small.
	for (iTank* tank : tanks) {
		if (tank->GetId() == ownerID) {
			sMessage bulletMessage;
			bulletMessage.command = "DESTROY SUPERBULLET";
			tank->RecieveMessage(bulletMessage);
			break;
		}
	}
...
void cCombatMediator::InitializeTanks() {
	...
	redTankMesh->positionXYZ = glm::vec3(x1, y2+ 3.f, 20.f);
...
void cCombatMediator::PromoteTank(int index) {
	if (index == 0) {
		//PLAYER CANNOT BE PROMOTED
		return;
	}
	else {
		iTank* superTank = new cSuperTank(tanks[index]->GetMesh());
		superTank->SetReciever(this);
		tanks.push_back(superTank);
