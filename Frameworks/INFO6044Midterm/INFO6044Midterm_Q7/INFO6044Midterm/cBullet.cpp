...
cBullet::cBullet(int _ownerID, glm::vec3 _position, glm::vec3 _direction, float damageModifier) {
	ownerID = _ownerID;
...
bool cBullet::CheckValidMove() {
	if (p_Mediator) {
		...
		outgoingMessage.vec_sData.push_back("BULLET");
		...
		if (responseMessage.command == "CLEAR") {
			...
			outgoingMessage2.vec_sData.push_back("BULLET");
