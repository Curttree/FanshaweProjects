...
cFireworkObject* cFireworkBuilder::buildFirework(int fireworkNum, glm::vec3 position, glm::vec3 debrisDirection, glm::vec3 debrisColour) {
	...
	case(0):
		...
		result->fuse = cFuseFactory::Instance()->createFuse(TIMED_FUSE, result->particle, 120.f);
		break;
	case(1):
		result = cFireworkFactory::Instance()->createFireworkObject(1, position, determineDirection(-1.f,1.f) * 150.f);
		result->fuse = cFuseFactory::Instance()->createFuse(TIMED_FUSE, result->particle, 40.f);
		break;
	case(2):
		result = cFireworkFactory::Instance()->createFireworkObject(2, position, determineDirection(-3.f, 3.f) * 40.f);
		result->fuse = cFuseFactory::Instance()->createFuse(APEX_FUSE, result->particle, 0.f);
		break;
	case(3):
	...
	default:
		...
		result->fuse = cFuseFactory::Instance()->createFuse(TIMED_FUSE, result->particle, 0.f);
		break;
	}
