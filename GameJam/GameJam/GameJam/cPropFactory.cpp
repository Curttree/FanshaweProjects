...
cProp* cPropFactory::createProp(int type, glm::vec3 position, glm::vec3 orientation, glm::vec3 velocity) {
	...
	switch (type) {
	case(PROP_CAN):
		result = ::g_pGameEngine->entityManager.CreateProp("can.ply", "BrainNerve.bmp", position, eShapeType::Cylinder,glm::vec3(0.5f),glm::vec3(1.5f,1.f,1.5f),3.f, orientation);
