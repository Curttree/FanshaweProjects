...
sFrustum cFrustumCullingHandler::createFromCamera(cFlyCamera* camera) {
    ...
    const float halfVSide = camera->farPlane * tanf(camera->FOV * .5f);
    const float halfHSide = halfVSide * aspectRatio;
    const glm::vec3 frontMultFar = camera->farPlane * camera->getAt();
    const glm::vec3 cameraRight = glm::normalize(glm::cross(camera->getUp(), camera->getAt()));

    result.nearFace = { glm::length(camera->getEye() + camera->nearPlane * camera->getAt()), camera->getAt() };
    result.farFace = { glm::length(camera->getEye() + frontMultFar), -camera->getAt() };
    result.rightFace = { glm::length(camera->getEye()), glm::normalize(glm::cross(camera->getUp(),frontMultFar + cameraRight * halfHSide)) };
    result.leftFace = { glm::length(camera->getEye()), glm::normalize(glm::cross(frontMultFar - cameraRight * halfHSide, camera->getUp())) };
    result.topFace = { glm::length(camera->getEye()), glm::normalize(glm::cross(cameraRight, frontMultFar - camera->getUp() * halfVSide)) };
    result.bottomFace = { glm::length(camera->getEye()), glm::normalize(glm::cross(frontMultFar + camera->getUp() * halfVSide, cameraRight)) };
	return result;
}
...
bool cFrustumCullingHandler::isWithinFrustum(const sFrustum cameraFrustum, const cMesh* mesh) {
    ...
    bool close = isWithinPlaneBounds(cameraFrustum.nearFace, mesh);
    bool away = isWithinPlaneBounds(cameraFrustum.farFace, mesh);
    return (close && away);
...
bool cFrustumCullingHandler::isWithinPlaneBounds(const sPlane plane, const cMesh * mesh) {
