...
cPlanet::cPlanet(float _x, float _y, float _radius, float _mass) {
	...
	surfaces.push_back(new Surface{- radius, - radius, - radius, -radius * 0.4f, PI/2.f });

	//Cave
	surfaces.push_back(new Surface{ -radius, -radius * 0.4f, -radius * 0.5f, -radius * 0.4f, 0.f });
	surfaces.push_back(new Surface{ -radius * 0.5f, -radius * 0.4f, -radius * 0.5f, radius * 0.4f, PI / 2.f });
	surfaces.push_back(new Surface{ -radius * 0.5f, radius * 0.4f, -radius, radius * 0.4f, PI});
	surfaces.push_back(new Surface{ -radius, radius * 0.4f, -radius, radius, PI / 2.f });
...
void cPlanet::GetSurfacePosition(Vec2& location, float& angle) {
	...
	while (!found && numIterations < maxIterations) {
		found = true;
		int surfaceIndex = gGetRandBetween(0, (int)surfaces.size()-1);
...
void cPlanet::GenerateSurfaces(Vec2 startPos, Vec2 endPos, float maxOffset, int segments) {
	...
	//TO BE EXTENDED. Since surfaces just need a start/end position and facing direction this data could either be generated randomly, or read from an external file if desired.
}