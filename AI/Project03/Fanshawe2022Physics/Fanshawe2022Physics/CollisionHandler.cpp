...
namespace gdp2022Physics
{
	...
	bool CollisionHandler::CollideSphereSphere(float deltaTime, cRigidBody* a_body, SphereShape* a_sphere, cRigidBody* b_body, SphereShape* b_sphere) {
		...
		glm::vec3 a_momentum = a_body->velocity * a_body->mass;
		glm::vec3 b_momentum = b_body->velocity * b_body->mass;
		...
		a_body->velocity -= (a_elasticMomenum + a_inelasticMomenum) * (a_body->invMass * a_body->restitution);
		b_body->velocity += (b_elasticMomenum + b_inelasticMomenum) * (b_body->invMass * b_body->restitution);
	...
	bool CollisionHandler::CollideSpherePlane(float deltaTime, cRigidBody* sphere, SphereShape* sphereShape,
		...
		float velocityLength = glm::length(sphere->velocity);

		if (velocityLength > 0.000001f)
		{

			float velocity = glm::length(sphere->velocity);
			...
			// Bouncing off
			glm::vec3 reflect = glm::reflect(sphere->velocity, planeShape->GetNormal());

			glm::vec3 prevVelocity = sphere->velocity;
			sphere->velocity = reflect;

			glm::vec3 impactComponent = ProjectOn(sphere->velocity, planeShape->GetNormal());
			glm::vec3 impactTangent = prevVelocity - impactComponent;
			sphere->ApplyForce(impactTangent * -1.f * sphere->mass * plane->friction);
			...
			if (penetration < sphereShape->GetRadius())
			{
				...
				float velDotNorm = glm::dot(sphere->velocity, planeShape->GetNormal());

				if (velDotNorm < 0.f)
				{
					sphere->velocity -= planeShape->GetNormal() * velDotNorm;
				}
			}
			else
			{
