...
//    ::g_pTheLights->theLights[0].param1.x = 1.0f;    // Spot light
    ...
    vecModelsToLoad.push_back("wall.ply");
...
#pragma region Project04
    ...
    for (unsigned int index = 0; index < mapManager.currentGraph->nodes.size(); index++) {
        if (mapManager.currentGraph->nodes[index]->terrain == Terrain::Start) {
            ...
            cVehicle tempPlayer(glm::vec3(pos.x, 0.f, pos.y));
            mapManager.vehicles.push_back(tempPlayer);
        }
    }
...
//        ::g_vecMeshes[5].positionXYZ = gTheLights.theLights[0].position;
        ...
        for (unsigned int x = 0; x < mapManager.vehicles.size(); x++) {
            mapManager.vehicles[x].Update(deltaTime);
        }
