...
int main(void) {
    ...
    pWindow = glfwCreateWindow(1200, 640, "Curtis Tremblay - INFO-6023 - Midterm", NULL, NULL);
...
#pragma region QUESTION_1 Maze
    ::g_pmazeMaker->GenerateMaze(50, 50);
    for (unsigned int x = 0; x < ::g_pmazeMaker->maze.size(); x++) {
        for (unsigned int y = 0; y < ::g_pmazeMaker->maze[x].size(); y++) {
            if (::g_pmazeMaker->maze[x][y][0]) {
                cMesh* grass = new cMesh;
                grass->meshName = "tile.ply";
                grass->textureNames[0] = "grass.bmp";
                grass->textureRatios[0] = 1.f;
                grass->friendlyName = "Grass_"+std::to_string(x)+"_"+ std::to_string(y);
                grass->positionXYZ = glm::vec3(x * 1.0f + 0.5f , -0.5f, y * 1.0f + 0.5f);
                grass->scale = 1.0f;

                g_vec_pMeshes.push_back(grass);
            }
            else {
                cMesh* path = new cMesh;
                path->meshName = "tile.ply";
                path->textureNames[0] = "path.bmp";
                path->textureRatios[0] = 1.f;
                path->friendlyName = "Path_" + std::to_string(x) + "_" + std::to_string(y);
                path->positionXYZ = glm::vec3(x * 1.0f + 0.5f, -0.5f, y * 1.0f + 0.5f);
                path->scale = 1.0f;

                g_vec_pMeshes.push_back(path);
            }
        }
    }
#pragma endregion
...
//        ::g_vecMeshes[5].positionXYZ = gTheLights.theLights[0].position;
        ...
        ::g_pEcoSystemManager->TimeStep(deltaTime);
