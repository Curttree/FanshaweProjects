...
{
	cCurtMap< std::string /*name*/, unsigned int /*ID*/ >::iterator
...
cShaderManager::pGetShaderProgramFromFriendlyName(std::string friendlyName)
{
	...
	cCurtMap< unsigned int /*ID*/, cShaderProgram >::iterator
...
bool cShaderManager::m_compileShaderFromSource(cShaderManager::cShader& shader, std::string& error)
{
	...
	unsigned int numberOfLines = static_cast<unsigned int>(shader.vecSource.get_size());
