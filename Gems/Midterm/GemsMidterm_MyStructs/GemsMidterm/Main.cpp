...
//    ::g_pTheLights->theLights[0].param1.x = 1.0f;    // Spot light
    ...
    cCurtArray<std::string> vecModelsToLoad;
    ...
    for (unsigned int index = 0; index < vecModelsToLoad.get_size(); index++)
    {
        std::string itModel = vecModelsToLoad.getAtIndex(index);
...
//        ::g_vecMeshes[5].positionXYZ = gTheLights.theLights[0].position;
        ...
        // **********************************************************************
        for (unsigned int index = 0; index != ::g_vec_pMeshes.get_size(); index++)
        {
            // So the code is a little easier...
            cMesh* pCurrentMesh = ::g_vec_pMeshes.getAtIndex(index);
